...
export default function Register() {
  const history = useNavigate();
    const checkError=(username, password,repeatPassword, name,birthday,identifyCard)=>{
      if(username.length < 6){
        
        document.querySelector(".UserName-Error").innerHTML = "User Name is not valid!"
        return true;
      }
      if(password.length < 6){
        
        document.querySelector(".Password-Error").innerHTML = "Password is not valid!"
         return true;
      }
      if(repeatPassword !== password){
         
        document.querySelector(".RepeatPassword-Error").innerHTML = "Password and Repeat is not fit"
         return true;
      }
      if(!birthday){
        
         document.querySelector(".Birthday-Error").innerHTML = "Birthday is not valid!"
          return true;
      }
      if(name.length < 1){
      ...
      if(/^\d+$/.test(identifyCard) && identifyCard.length != 9){
         ...
         document.querySelector(".IdentifyCard-Error").innerHTML = "IdentifyCard is not valid!"
          return true;
      }
      return false;
    }
    return (
        ...
        <div class="register_form">
          <h2>Register Form</h2>
          <form class="form_register" onSubmit={async(e)=>{
            e.preventDefault();
            ...
            const identifyCard = document.querySelector("#IdentifyCard").value;
            if(!checkError(username, password,repeatPassword,name,birthday,identifyCard)){
